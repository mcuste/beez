...
| `command`     | A program and its arguments as a list. Loom runs it without a shell.    |
...
A harness is a coding agent program. Loom starts it in a mode that takes one
...
Codex refuses to start outside a Git repository. Loom does not pass
...
file, or a command argument. Loom fills in the text when the task starts,
...
- Loom removes newlines at the end of the output, so a one-line result fits
  inside a sentence.
