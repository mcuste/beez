//! The shape of one line of `run.log`, shared with Beez's terminal output.
...
/// One of Beez's own lines: the status word in its own column, then the message.
#[must_use]
pub fn status_line(verb: &str, message: &str) -> String {
    format!("{verb:<VERB_WIDTH$}  {message}")
}

/// One of Beez's own lines with the time in front, as a log writes it.
#[must_use]
...
/// A duration as Beez writes it in a status line.
#[must_use]
...
/// What Beez reports when it cannot write the artifacts of a run.
#[must_use]
