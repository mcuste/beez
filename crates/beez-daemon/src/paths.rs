...
/// Directory of the daemon's own files, inside the Beez root.
...
///
/// A daemon belongs to one Beez root. The root holds the daemon's own files
...
impl DaemonPaths {
    /// The one daemon of the person running Beez, in `~/.beez`.
    ///
    ...
    pub fn user() -> io::Result<Self> {
        ...
        Ok(Self::new(home.join(".beez")))
    }

    /// Uses `root` as the Beez root, in place of the one Beez would find.
    #[must_use]
    ...
    /// The Beez root, which also holds the runs.
    #[must_use]
    ...
    /// Creates the daemon directory, and keeps the Beez root out of Git.
    pub fn create(&self) -> io::Result<()> {
        std::fs::create_dir_all(self.directory())?;

        beez_record::ignore_everything(&self.root)
    }
}
...
fn socket_path(directory: &Path) -> PathBuf {
    ...
    std::env::temp_dir().join(format!("beez-{:016x}.sock", fingerprint(directory)))
}
...
mod tests {
    ...
    fn keeps_the_daemon_of_a_person_in_their_home_directory() {
        ...
        assert_eq!(paths.root(), home.join(".beez"));
    }

    #[test]
    fn keeps_every_file_inside_the_beez_root() {
        let paths = DaemonPaths::new("/work/.beez");

        assert_eq!(paths.directory(), Path::new("/work/.beez/daemon"));
        assert_eq!(paths.socket(), Path::new("/work/.beez/daemon/daemon.sock"));
        assert_eq!(paths.record(), Path::new("/work/.beez/daemon/daemon.json"));
        assert_eq!(
            paths.manifests(),
            Path::new("/work/.beez/daemon/manifests.json")
        );
    }
    ...
    fn moves_a_socket_that_does_not_fit_out_of_a_deep_root() {
        let deep = format!("/{}/.beez", "directory/".repeat(20));
