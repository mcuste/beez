...
fn beez(root: &Path, arguments: &[&str]) -> io::Result<Output> {
    common::beez()
        .args(arguments)
        .arg("--root")
        .arg(root)
...
fn start(root: &Path) -> io::Result<Running> {
    let output = beez(root, &["daemon", "start"])?;
...
fn watches_a_manifest_before_the_daemon_starts() {
    ...
    let listed = beez(&root, &["schedule", "list"]).unwrap();
...
fn pauses_and_removes_a_job_before_the_daemon_starts() {
    ...
    let paused = beez(&root, &["schedule", "pause", "nightly"]).unwrap();
    let held = beez(&root, &["schedule", "list"]).unwrap();
    let resumed = beez(&root, &["schedule", "resume", "nightly"]).unwrap();
    let released = beez(&root, &["schedule", "list"]).unwrap();
    let removed = beez(&root, &["schedule", "remove", &path]).unwrap();
    let again = beez(&root, &["schedule", "remove", &path]).unwrap();
    let listed = beez(&root, &["schedule", "list"]).unwrap();
...
fn runs_a_job_on_request_and_writes_the_same_artifacts_as_a_run() {
    ...
    let triggered = beez(&root, &["schedule", "trigger", "nightly"]).unwrap();
    wait_for(&root, "ran the job", || marker.exists()).unwrap();
    wait_for(&root, "recorded the run", || {
        beez(&root, &["schedule", "list"]).is_ok_and(|output| stdout(&output).contains("(ok)"))
    })
    .unwrap();
    drop(daemon);
...
fn fires_a_one_time_schedule_once() {
    ...
    let later = beez_schedule::format_instant(std::time::SystemTime::now() + LIMIT);
    let path = directory
        .write(
            "once.yaml",
    ...
    wait_for(&root, "watched the manifest", || {
        beez(&root, &["schedule", "list"]).is_ok_and(|output| stdout(&output).contains(&later))
    })
    .unwrap();

    let at = beez_schedule::format_instant(std::time::SystemTime::now() + LEAD);
    ...
    wait_for(&root, "read the new fire time", || {
        beez(&root, &["schedule", "list"]).is_ok_and(|output| stdout(&output).contains(&at))
    })
    .unwrap();

    wait_for(&root, "fired the one-time schedule", || marker.exists()).unwrap();
    wait_for(&root, "finished the run", || {
        beez(&root, &["schedule", "list"]).is_ok_and(|output| stdout(&output).contains("done"))
    })
    .unwrap();
    let listed = beez(&root, &["schedule", "list"]).unwrap();
    drop(daemon);
...
fn runs_a_fire_it_missed_only_when_the_schedule_asks_for_it() {
    ...
    let at = beez_schedule::format_instant(std::time::SystemTime::now() - Duration::from_secs(60));
...
fn reads_a_manifest_again_after_it_changes() {
    ...
    wait_for(&root, "read the manifest again", || {
        beez(&root, &["schedule", "list"]).is_ok_and(|output| stdout(&output).contains("0 5 * * *"))
    })
    .unwrap();
    let listed = beez(&root, &["schedule", "list"]).unwrap();
    drop(daemon);
...
fn keeps_the_history_of_a_job_whose_manifest_stops_loading() {
    ...
    add(&root, &path).unwrap();
    let daemon = start(&root).unwrap();
    beez(&root, &["schedule", "trigger", "nightly:weekday"]).unwrap();
    wait_for(&root, "recorded the run", || {
        beez(&root, &["schedule", "list"]).is_ok_and(|output| stdout(&output).contains("(ok)"))
    })
    .unwrap();
    ...
    wait_for(&root, "reported the broken manifest", || {
        beez(&root, &["schedule", "list"]).is_ok_and(|output| stdout(&output).contains("broken"))
    })
    .unwrap();
    fs::write(&path, &source).unwrap();
    wait_for(&root, "read the fixed manifest again", || {
        beez(&root, &["schedule", "list"]).is_ok_and(|output| stdout(&output).contains("waiting"))
    })
    .unwrap();
    let listed = beez(&root, &["schedule", "list"]).unwrap();
    drop(daemon);
...
fn reads_the_list_of_watched_manifests_again_on_request() {
    ...
    let path = directory
        ...
        .unwrap();
    let daemon = start(&root).unwrap();
    let before = beez(&root, &["schedule", "list"]).unwrap();
    ...
    .unwrap();
    let reloaded = beez(&root, &["daemon", "reload"]).unwrap();
    let after = beez(&root, &["schedule", "list"]).unwrap();
    drop(daemon);
...
fn holds_a_paused_job_back_and_lets_it_go_again() {
    ...
    let resumed = beez(&root, &["schedule", "resume", "nightly"]).unwrap();
    let released = beez(&root, &["schedule", "list"]).unwrap();
    let unknown = beez(&root, &["schedule", "pause", "nothing"]).unwrap();
    drop(daemon);
...
fn keeps_a_second_daemon_out_of_one_root() {
    ...
    let second = beez(&root, &["daemon", "start"]).unwrap();
    drop(daemon);
