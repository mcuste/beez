...
//!
//! The Linux sandbox re-executes the `beez` binary as the first process inside
...
/// Runs `beez run command` with `arguments` in front of the program.
fn run_command(
...
fn sandboxes_a_command_unless_it_asks_for_no_sandbox() {
    ...
    let outside = outside_path("beez-sandbox-command.txt");
...
fn sandboxes_a_workflow_that_names_no_sandbox() {
    ...
    let outside = outside_path("beez-sandbox-default-leak.txt");
    let source = format!(
...
fn runs_a_task_that_opts_out_without_the_sandbox() {
    ...
    let outside = outside_path("beez-sandbox-opt-out.txt");
    let source = format!(
...
fn allows_writes_inside_the_working_directory_only() {
    ...
    let outside = outside_path("beez-sandbox-leak.txt");
...
fn refuses_local_connections_directly_and_through_the_proxy() {
    ...
    // Beez's own notes carry a status word, so a log never confuses them with
