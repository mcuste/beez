//! Tests the artifacts Beez writes for every run.
...
/// A plain run relays the bytes of its only task, so Beez adds no line of its
/// own, and the artifacts still hold the whole run.
#[test]
fn keeps_a_single_task_run_free_of_beez_lines_and_still_records_it() {
...
fn runs_a_harness_prompt_and_records_it() {
    ...
    let root = directory.join("artifacts");
    beez_test_support::fake_harness(&directory, "pi", 0).unwrap();

    let output = common::beez()
        .args(["run", "pi", "inspect the repository"])
        .env("PATH", beez_test_support::extended_path(directory.path()))
        .env("BEEZ_LOG_DIR", &root)
        .output()
        .unwrap();
