//! Checks that installed harnesses accept Beez's arguments.
#![cfg(unix)]

use std::process::{Command, Output, Stdio};

const UNKNOWN_PROVIDER: &str = "beez_contract_check";

#[test]
fn pi_accepts_the_flags_beez_sends() {
    assert_help_documents("pi", &[], &["--print", "--model", "--effort"]);
}

#[test]
fn omp_accepts_the_flags_beez_sends() {
    assert_help_documents("omp", &[], &["--print", "--model", "--thinking"]);
}

#[test]
fn claude_accepts_the_flags_beez_sends() {
    assert_help_documents("claude", &[], &["--print", "--model", "--effort"]);
}

#[test]
fn codex_accepts_the_flags_beez_sends() {
    assert_help_documents("codex", &["exec"], &["--model", "-c, --config"]);
}

#[test]
fn codex_accepts_the_effort_override_beez_sends() {
...
fn codex_rejects_an_unknown_config_override() {
    let report = report(run_isolated_codex("beez_contract_unknown_key=1"));

    assert!(report.contains("unknown configuration field `beez_contract_unknown_key`"));
}
...
fn run_isolated_codex(config_override: &str) -> Output {
    run(
        ...
        &[
            ...
            "--ignore-user-config",
            "-c",
            "model_provider=beez_contract_check",
            "-c",
            config_override,
